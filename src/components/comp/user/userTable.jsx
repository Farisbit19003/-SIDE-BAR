...
export const UserTable = ({handleDelete,users,keyword,Searched}) => {
          ...
          <div className="overflow-x-auto flex flex-col justify-center">
            ...
            {users&&users.length<1?(
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <span className="mt-4 text-gray-500 text-lg font-semibold">No User Found</span>
                    </div>
                  </div>
                ):  <tbody>
             {users&&users.filter(Searched(keyword)).map((item, index) => (
                  <tr
                  ...
                  >
                    ...
                    <td className="px-4 py-2">{item.whatsapp&&item.whatsapp}</td>
                    <td className="px-4 py-2">{item.address&&item.address}</td>
                    <td className="px-2 py-2  justify-center flex  cursor-pointer">
                    <BiTrash onClick={()=>handleDelete(item._id)} size={25} color="red"/>
                    </td>
                  </tr>
                ))}
              </tbody>}
