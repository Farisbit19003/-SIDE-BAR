...
import { AllShops, ApproveStore, DisApproveStore } from "../Create Shop/functions";
...
const handleDisApprove=(_id)=>{
  ...
  return (
    <>
    {!shops || shops.length===0?  <div className="flex  justify-center">
                    <div className="flex flex-col items-center">
                      {/* <AiOutlineLoading3Quarters className="text-6xl w-16 h-16 text-[#248F59] animate-spin" /> */}
                      <span className="mt-4 text-gray-500 text-lg font-semibold">Loading...</span>
                      <span className="mt-4 text-gray-500 text-lg font-semibold">No Shops Found</span>
                    </div>
                  </div> : <div className="my-6  flex border bg-white shadow">
              ...
              <tbody>
                {shops&&shops.filter(Searched(keyword)).map((item, index) => (
                  ...
                  >
                   ...
                   {page==="Admin"?<> {item.status === "Active"
                        ? <TiDelete onClick={()=>handleDisApprove(item._id)} 
                         size={25} color="red" />
                        :<TbCircleCheck 
