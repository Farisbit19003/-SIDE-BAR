  ...
  return (
    <>
      <div className="px-11 py-5 border border-[#f2f2f2] rounded bg-white">
        ...
        <div className="mb-2 flex flex-col flex-wrap items-center justify-between gap-x-8 text-base font-bold  sm:flex-row lg:flex-nowrap">
          <div className="flex mb-2 p-2 items-center justify-center flex-col md:flex-row">
            <span className="block lg:mb-0 font-sans lg:inline-block lg:ltr:mr-4 lg:rtl:ml-4">
              Order-status :
            </span>
            <span className="bg-[#248F59] hover:scale-95 transition-transform uppercase rounded cursor-default hover:text-white flex items-center justify-center text-sm font-normal ml-1 p-2 text-[#f2f2f2] font-sans h-fit">
        ...
        <div className="md:w-full w-fit mx-auto p-2 border rounded shadow-sm border-[#f2f2f2]">
          {/* ORDER ID */}
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div>
              <h1 className="font-serif font-normal lg:text-xl sm:text-base text-[#248F59]">
                Order Id- #{singleOrder?._id}
              </h1>
            </div>
            {/* ORDER STATUS */}
            {singleOrder?.orderType === "Sales" &&
              singleOrder.orderStatus !== "cancelled" && singleOrder.orderStatus !== "cancelled"  && (
